...
function FetchButcheryShops()  {
  ...
  useEffect(() => {
    axios.get('http://meating-point.innofabrik.de/api/butchery_shops')
    .then(
      response => {
  ...
  return (
    <div>
      ...
      <Link to={`/butchery_shops/${item.id}`} className="p-4 md:w-1/3 cursor-pointer">
                  <div className="flex rounded-lg h-full bg-gray-500 p-8 flex-col">
                    ...
                    <div className="flex-grow">
                      <img className="w-20 h-20" src={`../../../assets/${item.id}.jpg`} alt={`Bild${item.id}`} />
